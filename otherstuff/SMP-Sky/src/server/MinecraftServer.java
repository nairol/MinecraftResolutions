    ...
    implements Runnable, ICommandListener
{
    ...
    private boolean startServer() throws UnknownHostException
    {
        ...
        logger.info("Starting minecraft server version Beta 1.6.5");
    ...
    private void initWorld(ISaveFormat isaveformat, String s, long l)
    {
        ...
        for(int i = 0; i < worldMngr.length; i++)
        {
            if(i == 0)
            {
                worldMngr[i] = new WorldServer(this, saveolddir, s, 0, l);
            } else if(i == 1)
            {
                worldMngr[i] = new WorldServerMulti(this, saveolddir, s, -1, l, worldMngr[0]);
            } else if(i == 2)
            {
            	worldMngr[i] = new WorldServerMulti(this, saveolddir, s, 1, l, worldMngr[0]);
            }
...
label0:
        for(int j = 0; j < worldMngr.length; j++)
        {
            ...
            if(j == -1 && !propertyManagerObj.getBooleanProperty("allow-nether", true))
            {
                continue;
            }
            if(j == 1 && !propertyManagerObj.getBooleanProperty("allow-sky", true))
            {
                continue;
            }
    ...
    private void doTick()
    {
        ...
        for(int j = 0; j < worldMngr.length; j++)
        {
            ...
            if(deathTime % 20 == 0)
            {
                configManager.func_28169_a(new Packet4UpdateTime(worldserver.getWorldTime()), worldserver.worldProvider.worldType);
            }
            worldserver.tick();
